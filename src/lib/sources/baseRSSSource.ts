...
import { fetchRSSFeedViaMCP } from "../services/mcpService";
      ...
      if (typeof window !== 'undefined') {
        try {
          ...
          if (!proxyError?.name?.includes('Timeout') && !proxyError?.name?.includes('Abort')) {
            console.warn(`⚠ ${this.config.name}: CORS proxy failed, trying MCP server...`, proxyError.message);
          }
          
          // Fallback to MCP server (replaces RSS2JSON)
          try {
            const mcpResult = await fetchRSSFeedViaMCP(this.config.url, this.config.name, this.config.category);
            
            if (mcpResult.success && mcpResult.articles.length > 0) {
              const maxArticles = this.config.maxArticles || 50;
              const limitedArticles = mcpResult.articles.slice(0, maxArticles);
              
              const latestUpdateTime = limitedArticles.length > 0 
                ? Math.max(...limitedArticles.map(a => a.publishedAt))
                : Date.now();
            ...
            if (!mcpError?.name?.includes('Timeout') && !mcpError?.name?.includes('Abort')) {
              console.warn(`⚠ ${this.config.name}: MCP server also failed`, mcpError.message);
            }
          }
        }
      } else {
