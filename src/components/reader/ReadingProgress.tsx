 ...
 */
export function ReadingProgress({ containerRef, targetElement }: ReadingProgressProps) {
  const [progress, setProgress] = useState(0);
  const [showScrollTop, setShowScrollTop] = useState(false);

  useEffect(() => {
    const element = targetElement || containerRef?.current || document.documentElement;
    if (!element) return;
