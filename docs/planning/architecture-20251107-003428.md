...
│                                                               │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐     │
│  │   Frontend   │  │   State      │  │   Content    │     │
│  │   (React)    │→ │   Management │→ │   Service    │     │
│  │              │  │   (Zustand)  │  │   (Fetch)    │     │
│  └──────────────┘  └──────────────┘  └──────────────┘     │
│         │                 │                    │            │
│         │                 │                    │            │
...
├── (feed)/
│   ├── feed/
│   │   └── page.tsx               # Content feed
│   ├── article/
│   │   └── [id]/
│   │       └── page.tsx           # Article reader view
│   └── search/
...
class IndexedDBCache {
  ...
  private async cleanupOldArticles(): Promise<void> {
    ...
    if (keys.length > 2000) {
      ...
      await Promise.all(
        toRemove.map((key) => this.articlesStore.removeItem(key))
      );
    }
  }
}
...
1. User clicks login → Reown modal opens
...
[Check IndexedDB Cache]
    ...
    └─ Cache Miss/Expired → [Fetch from Sources]
            ↓
    [Parallel Fetching]
    ├─ Hacker News API
    ├─ Product Hunt API
...
[Clerk User Created (Background)]
    ...
    └─ Or Client-Side: clerk.signIn.create()
            ↓
...
[User Views Auction]
    ↓
[Check USDT Balance]
    ├─ Insufficient → [Show Top-Up Option]
    └─ Sufficient → [Show Bid Form]
            ↓
...
```
App
├── Providers (Reown, Clerk, React Query, Zustand)
│   └── Layout
│       ├── Header
│       │   ├── Logo
│       │   ├── SearchBar
...
export function ArticleCard({ article, onUpvote, onBookmark, onShare }: ArticleCardProps) {
  ...
  return (
    <div className="article-card">
      ...
      <h3>{article.title}</h3>
      <p>{article.source} • {formatTime(article.publishedAt)}</p>
      <div className="actions">
        ...
        <button onClick={() => onShare(article.id)}>📤</button>
      </div>
    </div>
  );
}
